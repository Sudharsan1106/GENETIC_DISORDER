...
def correlations(x, th=0.5):
   cors = x.corr()
   col_corr=set()    
   for i in range(len(cors.columns)):
       for j in range(i):
           if abs(cors.iloc[i,j])>th:
               colname=cors.columns[i]
...
def visualize_final(df1,y_name):
    # Data to plot
    labels = []
    sizes = []
    explodes=[]
    a=df1[y_name].value_counts()
    for x, y in a.items():
        labels.append(x)
        sizes.append(y)

    explode_list = [0.1, 0.0, 0.2, 0.3, 0.5, 0.6, 0.4, 0.3,0.9,0.7]
    
    for i in range(0,len(a)):
        explodes.append(random.choice(explode_list))

    # Creating color parameters 
    colors = ( "orange", "cyan", "pink",
              "grey", "indigo", "beige", "brown", "red","black") 
